...
export function LoginForm({ returnTo = "/profil" }: { returnTo?: string }) {
  ...
  return <form className="auth-form" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); startTransition(async () => { const result = await loginAction({ email: String(data.get("email")), password: String(data.get("password")), remember: data.get("remember") === "on", returnTo }); setMessage(result.message); if (result.ok && result.redirectTo) window.location.assign(result.redirectTo); }); }}><label>E-posta<input name="email" type="email" autoComplete="email" required/></label><label>Parola<input name="password" type="password" autoComplete="current-password" required/></label><div className="form-between"><label className="check"><input name="remember" type="checkbox"/> Beni hatırla</label><Link href="/sifremi-unuttum">Parolamı unuttum</Link></div>{message && <p className="form-message">{message}</p>}<button className="button primary full" disabled={pending}>{pending ? "Giriş yapılıyor…" : "Giriş yap"}</button><p className="form-alt">Hesabın yok mu? <Link href="/kayit">Ücretsiz kaydol</Link></p></form>;
}
