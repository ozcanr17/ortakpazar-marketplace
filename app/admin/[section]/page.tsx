...
export default async function AdminSectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  if (section === "settings") {
    const row = await database().prepare("SELECT commission_type,percentage_basis_points,fixed_fee_kurus,minimum_fee_kurus,maximum_fee_kurus,dispute_period_hours,seller_shipping_deadline_hours,buyer_confirmation_period_hours,prohibited_categories,maintenance_mode FROM platform_settings WHERE id = 1").first<{ commission_type: "PERCENTAGE" | "FIXED" | "HYBRID"; percentage_basis_points: number; fixed_fee_kurus: number; minimum_fee_kurus: number; maximum_fee_kurus: number | null; dispute_period_hours: number; seller_shipping_deadline_hours: number; buyer_confirmation_period_hours: number; prohibited_categories: string; maintenance_mode: number }>();
    if (!row) return <p>Ayarlar bulunamadı.</p>;
    return <><div className="admin-heading"><h1>Platform ayarları</h1><p>Komisyon oranı ve operasyon sürelerini yönetin.</p></div><SettingsForm settings={{ ...row, maintenance_mode: row.maintenance_mode === 1, prohibited_categories: JSON.parse(row.prohibited_categories) as string[] }}/></>;
  }
  if (section === "legal") {
